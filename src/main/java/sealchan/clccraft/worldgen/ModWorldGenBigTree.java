...
public class ModWorldGenBigTree extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    List<ModWorldGenBigTree.FoliageCoordinates> foliageCoords;
    
    private static Block ModLog;
    private static IBlockState ModLeaf;

    public ModWorldGenBigTree(boolean notify, Block log, Block leaf)
    {
        super(notify);
        
		ModLog = log;
		ModLeaf = leaf.getDefaultState();
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<ModWorldGenBigTree.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new ModWorldGenBigTree.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new ModWorldGenBigTree.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), ModLeaf.withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeaves()
    {
        for (ModWorldGenBigTree.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
    ...
    void generateLeafNodeBases()
    {
        for (ModWorldGenBigTree.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
            ...
            {
                this.limb(blockpos, worldgenbigtree$foliagecoordinates, ModLog);
            }
        }
    }
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        //*/
        /* Original Code
        if (this.heightLimit == 0) // Why Spongebob, why?...
        {
            this.heightLimit = 5 + this.rand.nextInt(this.heightLimitLimit);
        }
        //*/
